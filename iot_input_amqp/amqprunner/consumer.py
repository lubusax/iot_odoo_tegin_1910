...
class AmqpConsumer(object):
    ...
    """
    def __init__(self, amqp_url, exchange='odoo', exchange_type='topic',
                 queue='odoo', routing_key='#', db=False,
                 scheme='http', host='localhost', port=8069, user=False,
                 password=False):
    ...
    def on_message(self, channel, basic_deliver, properties, body):
        ...
        url = ('%s://%s:%s/iot/%s/action_amqp' % (
            self._scheme, self._host, self._port, basic_deliver.routing_key))
        self.acknowledge_message(basic_deliver.delivery_tag)
        data = {
            'delivery_tag': basic_deliver.delivery_tag,
            'routing_key': basic_deliver.routing_key,
            'properties': properties.__dict__,
            'basic_deliver': basic_deliver.__dict__,
